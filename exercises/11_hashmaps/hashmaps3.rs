...
fn build_scores_table(results: &str) -> HashMap<&str, TeamScores> {
    ...
    for line in results.lines() {
        ...
        if scores.contains_key(team_1_name) {
            let score = scores.get(team_1_name).unwrap();
            let updated_score = TeamScores { 
                goals_scored: team_1_score + score.goals_scored,
                goals_conceded: team_2_score + score.goals_conceded,
            };
            scores.insert(team_1_name, updated_score);
        } else {
            scores.insert(team_1_name, TeamScores { goals_scored: team_1_score, goals_conceded: team_2_score });
        }

        if scores.contains_key(team_2_name) {
            let score = scores.get(team_2_name).unwrap();
            let updated_score = TeamScores { 
                goals_scored: team_2_score + score.goals_scored,
                goals_conceded: team_1_score + score.goals_conceded,
            };
            scores.insert(team_2_name, updated_score);
        } else {
